...
class AccessPoint:
    def __init__(self, wlan='wlan0', inet=None, ip='192.168.45.1', netmask='255.255.255.0', ssid='MyAccessPoint',channel=1):
        self.wlan = wlan
        self.inet = inet
        self.ip = ip
    ...
    def _write_hostapd_config(self):
        with open(self.hostapd_config_path, 'w') as hostapd_config_file:
            hostapd_config_file.write(config.format(self.ssid, self.wlan,self.channel))
    ...
    def _start_router(self):
        ...
        # start dnsmasq
        s = 'dnsmasq --dhcp-authoritative --interface={} --dhcp-range={}.20,{}.100,{},4h --address=/gmail.com/{} '\
            .format(self.wlan, ipparts, ipparts, self.netmask,self.ip)
        self._execute_shell('echo "{}  gmail.com www.gmail.com mail.gmail.com" >> /etc/hosts'.format(self.ip))
